...
cap = cv2.VideoCapture('videos/forPVcut.mp4')
...
W_region = [420,457],[268,660],[232,1042],[1436,900],[1930,676],[1920,413],[1663,338],[110,407]
...
def showLoc(x1, y1, x2, y2, obj_id, type):
    global row_Array 
   
    if obj_id not in detected_IDs2:
        cx = int((x1 + x2) / 2)  # Calculate x-coordinate of the center
        cy = int((y1 + y2) / 2)  # Calculate y-coordinate of the center
        status = "Unparked"
        parknum = None
        # Check if the center lies inside any of the rectangles
        for i, area in enumerate(posList_A):
            check_firstPoint = cv2.pointPolygonTest(np.array(area), (cx, cy), True)
            if check_firstPoint >= 0:
                # Check if the bottom left of the bounding box hits or enters the polygon
        ...
        for i, area in enumerate(posList_B):
            ...
            if check_firstPoint >= 0:
                # Check if the bottom left of the bounding box hits the line or enters the polygon
        ...
        if status == "Unparked":
            row = [obj_id,None,'Unparked',obj_id,type,detected_IDs.get(obj_id),[x1,y1,x2,y2]]
            namestring = ['U' + str(obj_id)]
            row_Array = row_Array + namestring
            index = row_Array.index(namestring[0])
            row_dict[row_Array[index]] = row    
    ...
    for i, area in enumerate(posList_A):
        ...
        if check_firstPoint >= 0:
            ...
            if check_secondPoint >= 0:
                ...
                row_dict[row_Array[i]] = [i+1,area,'Unoccupied',0,0,0,[0,0,0,0]]
    ...
    for i, area in enumerate(posList_B):
        ...
        if check_firstPoint >= 0:
            ...
            if check_secondPoint >= 0:   
                ...
                row_dict[row_Array[i+7]] = [i+8,area,'Unoccupied',0,0,0,[0,0,0,0]]
                color = (0,255,0)   
...
def stream(cap):
    ...
    while(True):
        ...
        #comment if you want
        return array_Loc,row_dict#,array_Loc,parked_car,frame
